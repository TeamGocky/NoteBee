...
\begin{figure*}
\centering
\includegraphics[scale=0.6]{../imgs/walkthroughseqdiag.png}
\caption{Sequence diagram for Joe's Walkthrough described in
...
\item	On the architecture diagram, Identify and label the main
messages that will be parsed through the application.
\item	or alternatively (and preferably) include sequence diagrams to
denote the sequence of communications parse between clients and servers.
Figure~\ref{fig:seqdiag} illustrates a sequence diagram for the
walkthrough in Section~\ref{sec:walkthrough}.
